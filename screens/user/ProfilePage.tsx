...
import { Copy, KeyRound, RefreshCw, Save, ShieldCheck, ShieldOff, UserRound, AlertTriangle, Fingerprint, CircleUserRound, MessageCircle, MailCheck, LogOut } from 'lucide-react';
import { PanelPublicSettings, Session, User } from '../../lib/types';
import { btn, inp } from '../../lib/constants';
import { DashboardPage } from '../../components/layout/DashboardPage';
import { Field, PageHeader, Panel, cn, formControlClass } from '../../components/ui';
import { HeadersMap, readResponse, requestJson } from '../../lib/http';
import { useApiAction } from '../../hooks/useApiAction';
import { AccountActivity } from '../../components/server/ServerActivity';
import { clearCsrfToken, csrfHeaders } from '../../lib/csrf';

export function ProfileScreen({ apiBase, showToast, session, setSession, settings }: { apiBase: string; showToast: (msg: string, type: 'success' | 'error') => void; session: Session; setSession: (s: any) => void; settings: PanelPublicSettings }) {
  ...
  return (
    <ProfilePage
      ...
      onChangePassword={async (data) => {
        const result = await run(() => requestJson(apiBase, '/auth/password', {}, { method: 'PATCH', body: JSON.stringify(data) }), 'Password changed');
        if (result?.user) setSession({ user: result.user });
      }}
      onTwoFactorChanged={user => setSession({ user })}
      onRevokeAllSessions={async () => {
        if (!window.confirm('Sign out every active Agapornis session, including this one?')) return;
        const result = await run(async () => readResponse(await fetch('/api/auth/logout-all', {
          method: 'POST',
  ...
  return (
    <DashboardPage>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(22rem,28rem)]">
        ...
        <Panel className="overflow-hidden border-[var(--border)]/70 bg-[var(--card)] shadow-sm">
          ...
          >
            {user.passwordEnabled !== false && (
              <Field label="Current password">
                <input className={cn(inp, formControlClass())} type="password" value={passwords.currentPassword} onChange={event => setPasswords({ ...passwords, currentPassword: event.target.value })} />
              </Field>
            )}
            <Field label="New password">
              <input className={cn(inp, formControlClass())} type="password" minLength={settings.passwordPolicy.minLength} maxLength={settings.passwordPolicy.maxLength} value={passwords.newPassword} onChange={event => setPasswords({ ...passwords, newPassword: event.target.value })} />
              <p className="mt-1.5 text-xs text-[var(--muted-foreground)]">Use {settings.passwordPolicy.minLength}+ characters and at least {settings.passwordPolicy.requiredCharacterClasses} character types.</p>
            </Field>
            <Field label="Confirm new password">
              <input className={cn(inp, formControlClass())} type="password" minLength={settings.passwordPolicy.minLength} maxLength={settings.passwordPolicy.maxLength} value={passwords.confirmPassword} onChange={event => setPasswords({ ...passwords, confirmPassword: event.target.value })} />
            </Field>
            ...
            <div className="pt-3">
              ...
              >
                ...
                {user.passwordEnabled === false ? 'Set Password' : 'Update Password'}
              </button>
            </div>
          </form>
        </Panel>
      ...
      <Panel className="overflow-hidden border-red-500/20 bg-red-500/5">
        <div className="flex flex-col gap-5 p-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="mt-2 text-sm leading-relaxed text-[var(--muted-foreground)]">
              Immediately invalidates every JWT issued for this account. Use this if a browser session or bearer token may have been stolen. You will need to sign in again on every device.
            </p>
          </div>
          <button
            className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-red-500 px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-red-600 disabled:cursor-not-allowed disabled:opacity-50"
            disabled={busy}
            onClick={() => void onRevokeAllSessions()}
          >
            <LogOut size={16} /> Revoke and sign out
          </button>
        </div>
      </Panel>
  ...
  return (
    <Panel className="overflow-hidden border-[var(--border)]/70 bg-[var(--card)] shadow-sm">
      ...
      <div className="flex items-center justify-between border-b border-[var(--border)]/60 bg-[var(--secondary)]/10 px-6 py-4">
        ...
        <div className={cn(
          "flex items-center gap-2 rounded-full border px-2.5 py-0.5  text-[10px] font-bold uppercase tracking-widest",
            ...
            <div className="flex flex-col gap-3">
              <div className="flex h-[220px] w-[220px] items-center justify-center rounded-xl bg-white p-2 shadow-[0_0_20px_-5px_rgba(0,0,0,0.3)] ring-1 ring-[var(--border)]">
                {qrCode ? (
                  ...
                  <span className=" text-xs text-black/50">Generating QR...</span>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-5 rounded-xl border border-[var(--border)]/40 bg-[var(--secondary)]/5 p-6">
              <div className="flex flex-col gap-1">
                ...
                <p className="text-xs text-[var(--muted-foreground)]">
                  You have <span className="text-[var(--foreground)]">{user.recoveryCodesRemaining || 0}</span> recovery codes remaining.
                </p>
              </div>
              
              <Field label="Current Authenticator Code">
                <input 
                  className={cn(inp, "bg-[var(--secondary)]/10 border-[var(--border)]/60 tracking-widest")} 
                  placeholder="000000"
...
function ConnectedAccountsPanel({
  ...
  const providers = [
    { key: 'google' as const, label: 'Google', icon: CircleUserRound, enabled: settings.socialAuth.google.enabled },
    { key: 'discord' as const, label: 'Discord', icon: MessageCircle, enabled: settings.socialAuth.discord.enabled }
  ];

  async function disconnect(provider: 'google' | 'discord') {
    setBusyProvider(provider);
    try {
      const next = await requestJson(apiBase, `/auth/social/${provider}`, authHeaders, { method: 'DELETE' });
      onChanged(next);
      showToast(`${provider === 'google' ? 'Google' : 'Discord'} disconnected`, 'success');
    } catch (error: any) {
      showToast(error.message, 'error');
    } finally {
  ...
  async function resendVerification() {
    ...
    try {
      await requestJson(apiBase, '/auth/email-verification/request', authHeaders, { method: 'POST' });
  ...
  return (
    <Panel className="overflow-hidden border-[var(--border)]/70 bg-[var(--card)] shadow-sm">
      <div className="flex items-center gap-3 border-b border-[var(--border)]/60 bg-[var(--secondary)]/10 px-6 py-4">
        <CircleUserRound size={16} className="text-[var(--primary)]" />
        <span className="text-xs font-semibold uppercase tracking-widest text-[var(--muted-foreground)]">Connected Accounts</span>
      </div>
      <div className="grid gap-4 p-6 md:grid-cols-2">
        {providers.map(({ key, label, icon: Icon, enabled }) => {
          const connected = user.authProviders?.includes(key) === true;
          return (
            <div key={key} className="flex items-center justify-between gap-4 rounded-xl border border-[var(--border)]/60 bg-[var(--secondary)]/5 p-4">
              <div className="flex items-center gap-3">
                <Icon size={20} className="text-[var(--primary)]" />
                <div>
                  <p className="text-sm font-bold">{label}</p>
                  <p className="text-xs text-[var(--muted-foreground)]">{connected ? 'Connected' : enabled ? 'Available to connect' : 'Disabled by administrator'}</p>
                </div>
              </div>
              {connected ? (
                <button className={cn(btn, 'text-xs')} disabled={busyProvider === key} onClick={() => void disconnect(key)}>Disconnect</button>
              ) : (
                <button
                  className={cn(btn, 'text-xs')}
                  disabled={!enabled}
                  onClick={() => window.location.assign(`/api/auth/oauth/start?provider=${key}&mode=link`)}
                >
                  Connect
                </button>
              )}
        ...
        {settings.accountSecurity.emailVerificationRequired && (
          <div className="flex items-center justify-between gap-4 rounded-xl border border-[var(--border)]/60 bg-[var(--secondary)]/5 p-4 md:col-span-2">
            <div className="flex items-center gap-3">
              <MailCheck size={20} className={user.emailVerified ? 'text-[var(--success)]' : 'text-amber-400'} />
              <p className="text-sm font-medium">{user.emailVerified ? 'Email address verified' : 'Email verification is still required for this account.'}</p>
            </div>
            {!user.emailVerified && <button className={cn(btn, 'text-xs')} disabled={busyProvider === 'email'} onClick={() => void resendVerification()}>Resend</button>}
          </div>
        )}
      </div>
    </Panel>
