...
const fallbackSettings: PanelPublicSettings = {
  ...
  socialLinks: { website: '', discord: '', instagram: '', twitter: '', youtube: '', github: '', linkedin: '' },
  registration: { enabled: true, inviteRequired: false },
  accountSecurity: { emailVerificationRequired: false, suspiciousLoginDetection: true },
...
export async function loadPublicSettings(): Promise<PanelPublicSettings> {
  try {
    ...
    return {
      ...
      socialLinks: settings.socialLinks || fallbackSettings.socialLinks,
      ...
      accountSecurity: settings.accountSecurity || fallbackSettings.accountSecurity,
      passwordPolicy: settings.passwordPolicy || fallbackSettings.passwordPolicy
    };
  } catch {
