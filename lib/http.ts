...
export async function readResponse(res: Response) {
  ...
  if (!res.ok) {
    const error: any = new Error(data?.message || data?.error || data?.errorMessage || res.statusText);
...
export async function requestJson(apiBase: string, path: string, authHeaders: HeadersMap, opts: RequestInit = {}) {
  ...
  return readResponse(await fetch(`${apiBase || '/api'}${path}`, { ...opts, method, body, headers, credentials: 'same-origin' }));
}
