...
import { EmptyState, PageHeader, Panel, PanelTitleBar, cn, formControlClass } from '../ui';
...
<div className="mx-auto grid w-full max-w-[1400px] gap-6 px-3 pb-8 sm:px-4 sm:gap-8 lg:gap-10 lg:px-0 lg:pb-12">
      ...
      <PageHeader
        title="Network Nodes"
        description="Provision, authenticate, and manage edge nodes across the infrastructure."
      ...
      <div className="grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1fr)_400px] lg:gap-8 lg:items-start">
        ...
        <div className="flex min-w-0 flex-col gap-6 lg:gap-8">
          ...
          <Panel className="overflow-hidden border-[var(--border)]/60 bg-[var(--background)]/50 backdrop-blur-sm">
            <PanelTitleBar icon={<Network size={18} className="text-[var(--primary)]" />} title="Register New Node" />
                                    ...
                                    <>
                                      ...
                                      <div className="grid gap-2 border-t border-[var(--border)] p-3 md:hidden">
                                        {agentAllocations.map(row => (
                                          <div
                                            key={`${row.kind || 'server'}:${row.databaseId || row.serverId}:${row.port}`}
                                            ...
                                            <div className="flex min-w-0 items-start justify-between gap-3">
                                              <div className="min-w-0">
                                                ...
                                                <p className="mt-1 truncate text-[11px] text-[var(--muted-foreground)]">
                                                  {row.kind === 'database' ? `Database for ${row.serverId}` : `${row.serverId}${row.portVariable ? ` · ${row.portVariable}` : ''}`}
                                                </p>
                                              </div>
                                              ...
                                              >
                                                ...
                                                <td className="px-4 py-2">
                                                  ...
                                                  <span className="ml-2 text-[var(--muted-foreground)]">
                                                    {row.kind === 'database' ? `for ${row.serverId}` : `${row.serverId}${row.portVariable ? ` · ${row.portVariable}` : ''}`}
                                                  </span>
                                                </td>
