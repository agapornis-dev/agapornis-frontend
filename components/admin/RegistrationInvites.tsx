...
export function RegistrationInvites({ apiBase, showToast }: { apiBase: string; showToast: (message: string, type: 'success' | 'error') => void }) {
  ...
  return (
    <Panel className="mx-auto w-full max-w-[1400px] overflow-hidden border-[var(--border)]/60 bg-[var(--background)]/50 h-screen">
      ...
      <div className="grid gap-5 p-6 lg:grid-cols-[1fr_12rem_auto] lg:items-end">
      ...
      <div className="border-t border-[var(--border)]/60">
        {invitations.length === 0 ? <EmptyState className="py-8 text-center">No unused invitation keys.</EmptyState> : (
            ...
            {invitations.map(invitation => (
              <div key={invitation.id} className="flex items-center gap-4 px-6 py-3">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-[var(--muted-foreground)]">Expires {new Date(invitation.expiresAt).toLocaleString()}</p>
                </div>
                <button className={cn(ghostBtn, 'h-8 w-8 px-0 text-[var(--muted-foreground)] hover:text-[var(--destructive)]')} title="Revoke key" disabled={busy} onClick={() => void revoke(invitation)}><Trash2 size={15} /></button>
              </div>
            ))}
          </div>
        )}
