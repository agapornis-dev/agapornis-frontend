...
export function LocationsScreen({ apiBase, showToast }: { apiBase: string; showToast: (message: string, type: 'success' | 'error') => void }) {
  const { data: locations, loading, refresh } = useLazyData<any[]>(apiBase, '/locations', {}, []);
  ...
  const isCreateIdInUse = locations.some(loc => loc.id.toLowerCase() === form.id.trim().toLowerCase());
    ...
    <div className="mx-auto max-w-[1200px] grid gap-10 pb-12">
      ...
      <div className="flex flex-col gap-2">
        <h2 className="text-3xl font-extrabold tracking-tight text-[var(--foreground)] sm:text-4xl">
          Locations<span className="text-[var(--primary)]">.</span>
        </h2>
        <p className="text-sm font-medium text-[var(--muted-foreground)]/80">
          Group nodes by datacenter or region and describe where workloads will run.
        </p>
      </div>
      ...
      <Panel className="overflow-hidden border-[var(--border)]/60 bg-[var(--background)]/50 backdrop-blur-sm">
        <div className="flex items-center gap-3 border-b border-[var(--border)]/60 bg-[var(--secondary)]/10 px-6 py-4">
          ...
          <h3 className="text-sm font-bold tracking-wide text-[var(--foreground)]">Register New Location</h3>
        </div>
                        ...
                        {isEditing ? (
                          <>
                            <input 
                              ...
                              autoFocus
                            />
                            <p className="text-xs font-mono text-[var(--muted-foreground)] px-1 opacity-70">
                              {location.id}
                            </p>
                          </>
                        ) : (
                          <>
                            ...
                            <p className="text-xs font-mono text-[var(--muted-foreground)]">{location.id}</p>
                          </>
                        )}
                      </div>
                    </div>
