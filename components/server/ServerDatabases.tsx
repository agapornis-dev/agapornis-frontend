...
export function ServerDatabases({
  ...
  async function powerDatabase(database: ServerDatabase, action: 'start' | 'stop' | 'reset') {
    ...
    try {
      ...
      setDatabases(current => current.map(item => item.id === database.id ? { ...item, ...updated } : item));
    } catch (err: any) {
  ...
  async function revealCredentials(database: ServerDatabase) {
    setBusy(true);
    setError('');
    try {
      ...
      setDatabases(current => current.map(item => item.id === database.id ? { ...item, ...credentials } : item));
    } catch (err: any) {
      setError(err.message || 'Failed to reveal database credentials');
    } finally {
      setBusy(false);
    }
  }
                    ...
                    return (
                      <tr key={database.id} className="transition-colors hover:bg-[var(--secondary)]/5 group">
                        ...
                        <PasswordCell
                          ...
                          copied={copied === `${database.id}:password`}
                          disabled={busy}
                          canReveal={server.access?.relationship !== 'collaborator'}
                          onReveal={() => revealCredentials(database)}
                          onCopy={() => database.password && copy(database.password, `${database.id}:password`)}
                        />
...
function PasswordCell({
  ...
  if (!value) {
    return (
      <td className="px-5 py-4">
        <button
          className="rounded-md border border-[var(--border)] bg-[var(--secondary)]/10 px-2.5 py-1.5 text-[11px] font-semibold text-[var(--foreground)] transition-colors hover:bg-[var(--secondary)]/40 disabled:cursor-not-allowed disabled:opacity-50"
          disabled={disabled || !canReveal}
          onClick={onReveal}
          title={canReveal ? 'Reveal credentials for this session' : 'Only the server owner or a panel administrator can reveal credentials'}
        >
          {canReveal ? 'Reveal' : 'Restricted'}
        </button>
      </td>
    );
  }
  return <CopyCell value={value} copied={copied} onCopy={onCopy} isPassword />;
}
