...
type SettingsSection = 'runtime' | 'versions' | 'resources' | 'databases' | 'backups' | 'variables';
...
function isUserEditableVariable(variable: any) {
  ...
  return Boolean(key) && variable?.userEditable !== false && !RESOURCE_KEYS.has(key) && !PORT_VARIABLE_PATTERN.test(key);
}
...
function resourceDefaults(server: ServerRecord) {
  return {
    ...
    cpuLimitPercentage: String(server.cpuLimitPercentage || variableValue(server, 'SERVER_CPU') || '100'),
    cpuCores: String(server.cpuCores || variableValue(server, 'SERVER_CPU_CORES') || ''),
    ...
    databaseCpuLimitPercentage: String(server.databaseCpuLimitPercentage || 50),
    databaseCpuCores: String(server.databaseCpuCores || ''),
...
export function ServerVariables({
  ...
  const [section, setSection] = useState<SettingsSection>(canManageResources ? 'resources' : 'variables');
  ...
  const [versionForm, setVersionForm] = useState({ eggId: server.eggId || '', version: '', build: '', includeExperimental: false });
  ...
  const dockerImages = selectedEgg?.dockerImages?.length
    ? selectedEgg.dockerImages
    : (selectedEgg?.images || []).map((image: string) => ({ label: image, image }));
  ...
  const sections = useMemo(() => [
    ...(availableEggs.length > 0 && onChangeEgg ? [{ value: 'runtime', label: 'Runtime' }] : []),
    { value: 'versions', label: 'Versions', disabled: !versionTabEnabled, title: versionTabEnabled ? 'Browse game versions and install runtime updates' : 'This server does not have a version-aware egg' },
    ...(canManageResources ? [
  ...
  ], [availableEggs.length, canManageResources, onChangeEgg, versionTabEnabled]);
  ...
  }, [server.id, server.eggId, eggs, canManageResources, canEditServerId]);
  ...
  useEffect(() => {
    ...
    setVersionForm({ eggId: server.eggId || '', version: '', build: '', includeExperimental: false });
  ...
  const applyVersion = async () => {
    ...
    const images = egg?.dockerImages?.length ? egg.dockerImages : (egg?.images || []).map((image: string) => ({ image }));
    await onChangeEgg({
      eggId: egg.id,
      dockerImage: images[0]?.image,
      variables: {},
  ...
  const save = async () => {
    ...
    if (section === 'databases') {
      return onSave({
        ...
        databaseCpuCores: resources.databaseCpuCores,
    ...
    if (section === 'variables') return onSave({ variables: variablesFromRows(rows) });
  };
  ...
  return (
    <div className="mx-auto grid max-w-5xl gap-5 p-4 sm:p-6">
      ...
      {section === 'runtime' && availableEggs.length > 0 && onChangeEgg && (
        <SettingsCard icon={<Box size={18} />} title="Runtime environment" description="Change the server template, image, and install-time variables.">
    ...
    {versionLoading && !versionCatalog ? (
      ...
      <div className="flex flex-col gap-8">
        {(versionCatalog?.games || []).map((game: any) => {
          const isSelected = game.eggs.some((egg: any) => egg.eggId === versionForm.eggId);
          const selectedEgg = game.eggs.find((egg: any) => egg.eggId === versionForm.eggId);
          ...
          return (
            <div key={game.id} className="flex flex-col gap-3">
                ...
                {game.eggs.map((egg: any) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        if (selected) {
                          setVersionForm(current => ({ ...current, eggId: '', version: '', build: '' }));
                        } else {
                          setVersionForm(current => ({ ...current, eggId: egg.eggId, version: '', build: '' }));
              ...
              {/* Inline version/build panel — slides open when a fork is selected */}
              {selectedEgg && (
                <div className="mt-1 flex flex-col gap-4 rounded-lg border border-[var(--border)]/60 bg-[var(--secondary)]/5 p-4">
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-sm font-medium text-[var(--foreground)]">{selectedEgg.family}</p>
                      <p className="text-xs text-[var(--muted-foreground)]">
                        {game.name} · {selectedEgg.kind.replace('-', ' ')}
                      </p>
                    </div>
                    <button
                      className="rounded p-1 text-[var(--muted-foreground)] hover:bg-[var(--secondary)]/60 hover:text-[var(--foreground)] focus:outline-none"
                      onClick={() => setVersionForm(current => ({ ...current, eggId: '', version: '', build: '' }))}
                  ...
                  <label className="flex cursor-pointer items-center gap-2.5 w-fit">
                    <div className="relative flex items-center">
                      <input
                        ...
                        onChange={event =>
                          setVersionForm(current => ({ ...current, includeExperimental: event.target.checked }))
                        }
                      />
                  ...
                  {/* Version + build selectors */}
                  <div className="grid gap-4 sm:grid-cols-2">
                    <Field label={selectedEgg.versionLabel || 'Version'}>
                    ...
                    <Field label={selectedEgg.buildLabel || 'Build'}>
                      {visibleBuilds.length > 0 ? (
                        <select
                          ...
                          disabled={versionLoading}
                          onChange={event =>
                            setVersionForm(current => ({ ...current, build: event.target.value }))
                          }
                        >
      ...
      {section === 'resources' && canManageResources && (
        <SettingsCard icon={<Server size={18} />} title="Resource limits" description="CPU, memory, and disk assigned to the game server.">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <UnitField label="CPU limit" unit="%" icon={<Cpu size={16} />} value={resources.cpuLimitPercentage} onChange={value => setResources({ ...resources, cpuLimitPercentage: value })} />
            <UnitField label="CPU cores" icon={<Cpu size={16} />} value={resources.cpuCores} onChange={value => setResources({ ...resources, cpuCores: value })} placeholder="Optional" />
          </div>
        </SettingsCard>
      )}
...
{section === 'databases' && canManageResources && (
        ...
        >
          <div className={cn(
            ...
            {/* Container Resource Limits Section */}
            <div className="flex flex-col gap-4">
              <h4 className="text-xs font-semibold uppercase tracking-wider text-[var(--muted-foreground)]">
                Container Limits
              </h4>
              <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                <UnitField 
                  ...
                  value={resources.databaseMemoryMb} 
                  onChange={value => setResources({ ...resources, databaseMemoryMb: value })} 
                />
                <UnitField 
                  label="Database disk" 
                  unit="MB" 
                  icon={<HardDrive size={16} />} 
                  value={resources.databaseDiskMb} 
                  onChange={value => setResources({ ...resources, databaseDiskMb: value })} 
                />
                <UnitField 
                  label="Database CPU" 
                  unit="%" 
                  icon={<Cpu size={16} />} 
                  value={resources.databaseCpuLimitPercentage} 
                  onChange={value => setResources({ ...resources, databaseCpuLimitPercentage: value })} 
                />
                <UnitField 
                  label="Database cores" 
                  icon={<Cpu size={16} />} 
                  value={resources.databaseCpuCores} 
                  onChange={value => setResources({ ...resources, databaseCpuCores: value })} 
      ...
      {section === 'variables' && (
        <SettingsCard
          ...
          description={canManageResources ? 'Manage runtime variables without mixing them with resource or database controls.' : 'Only variables marked as user-editable by the egg are shown.'}
        ...
        >
          <div className="grid gap-3">
            {rows.length === 0 ? (
              ...
              return (
                <div key={`${row.key}:${index}`} className="grid gap-2 rounded-md border border-[var(--border)] bg-[var(--secondary)]/10 p-3 md:grid-cols-[220px_1fr_auto] md:items-start">
                  ...
                  <div>
                    <input className={cn(inp, 'font-mono text-sm')} value={row.value} placeholder="value" disabled={ownerOnly} onChange={event => updateRow(index, { value: event.target.value })} />
                  ...
                  {canManageResources && <button className={cn(ghostBtn, 'px-2 text-[var(--muted-foreground)] hover:text-[var(--destructive)]')} disabled={busy} onClick={() => setRows(current => current.filter((_row, rowIndex) => rowIndex !== index))} title="Remove variable"><Trash2 size={16} /></button>}
                </div>
              );
            })}
          </div>
      ...
      {section !== 'runtime' && section !== 'versions' && (
        <div className="flex items-center justify-end border-t border-[var(--border)] pt-4">
          <button className={cn(btn, 'gap-2')} disabled={busy || hasOwnerOnlyVariable} onClick={() => void save()}>
  ...
  return (
    <Field label={label}>
      <div className="relative">
        ...
        <input className={cn(inp, 'pl-9', unit && 'pr-12')} value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder} />
