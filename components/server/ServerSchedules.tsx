...
const ACTION_LABELS: Record<ServerSchedule['action'], string> = {
  ...
  backup_delete: 'Delete oldest backup',
  clear_directory: 'Clear folder contents',
};

const ACTION_SCOPES: Record<ServerSchedule['action'], string> = {
  restart: 'power',
  start: 'power',
  stop: 'power',
...
function formToPayload(f: FormState) {
  ...
  return {
    ...
    targetPath: f.action === 'clear_directory' ? f.targetPath.trim() : undefined,
    storage: f.action === 'backup_create' || f.action === 'backup_delete' ? f.storage : undefined,
    intervalSeconds,
    enabled: f.enabled,
  };
}
...
export function ServerSchedules({
  ...
  const fullAccess = server.access?.relationship !== 'collaborator' || server.access?.permission === 'operator';
  const allowedActions = (Object.keys(ACTION_LABELS) as ServerSchedule['action'][])
    .filter(action => fullAccess || granted.has(ACTION_SCOPES[action]));
  ...
  async function load(silent = false, signal?: AbortSignal) {
    ...
    try {
      const data: ServerSchedule[] = await requestJson(
        apiBase,
        base,
        authHeaders,
        { signal },
      ) as any;
      if (!signal?.aborted) setSchedules(Array.isArray(data) ? data : []);
    } catch (e: any) {
      if (!signal?.aborted && !silent) setMessage(e.message || 'Failed to load schedules');
    } finally {
  ...
  }, [apiBase, server.id, server.nodeId]);

  function openCreate() {
    setEditId(null);
    setForm({ ...blank, action: allowedActions[0] || 'restart' });
    setShowForm(true);
    setMessage('');
  }
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {!showForm && allowedActions.length > 0 && (
      ...
      {showForm && (
        <form onSubmit={save} className="rounded-lg border border-[var(--border)] bg-[var(--secondary)]/30 p-5 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {form.action === 'clear_directory' && (
              <div className="sm:col-span-2 space-y-1.5">
                <label className={label}>Folder to clear</label>
                <input
                  className={cn(inp, 'font-mono')}
                  placeholder="logs/archive"
                  value={form.targetPath}
                  required
                  onChange={e => setForm({ ...form, targetPath: e.target.value })}
                />
                <p className="text-xs text-[var(--destructive)]">All files and subfolders inside this folder will be permanently deleted. The server root cannot be selected.</p>
              </div>
            )}

            {(form.action === 'backup_create' || form.action === 'backup_delete') && (
              <div className="space-y-1.5">
                <label className={label}>Backup storage</label>
                <select className={inp} value={form.storage} onChange={e => setForm({ ...form, storage: e.target.value as 'local' | 's3' })}>
                  <option value="local">Local</option>
                  <option value="s3">S3</option>
                </select>
                {form.action === 'backup_delete' && <p className="text-xs text-[var(--muted-foreground)]">Deletes only the oldest backup in the selected storage.</p>}
              </div>
            )}
            ...
            {form.presetSeconds === 0 && (
              <div className="space-y-1.5">
                <label className={label}>Custom interval (seconds, min {['backup_create', 'backup_delete', 'clear_directory'].includes(form.action) ? 300 : 60})</label>
                <input
                  className={inp}
                  type="number"
                  min={['backup_create', 'backup_delete', 'clear_directory'].includes(form.action) ? 300 : 60}
                  placeholder="3600"
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center gap-3 py-16 text-center text-[var(--muted-foreground)]">
          ...
          <p className="text-xs">Create a schedule for power actions, commands, backups, or safe folder cleanup.</p>
        </div>
      ) : (
          ...
          {schedules.map(s => (
            ...
            >
              ...
              <button
                ...
                disabled={!allowedActions.includes(s.action)}
                onClick={() => toggle(s)}
                className={cn(
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-[var(--muted-foreground)] truncate">
                  ...
                  {s.action === 'command' && s.command ? `: ${s.command}` : ''}
                  {s.action === 'clear_directory' && s.targetPath ? `: ${s.targetPath}` : ''}
                  {(s.action === 'backup_create' || s.action === 'backup_delete') ? ` (${s.storage || 'local'})` : ''}
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                <button
                  ...
                  disabled={runningId === s.id || !allowedActions.includes(s.action)}
                  onClick={() => runNow(s)}
                >
                  {runningId === s.id
